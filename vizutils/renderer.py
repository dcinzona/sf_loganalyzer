...
class renderer():
    ...
    def __init__(self, filename:str='') -> None:
        self.g = graphviz.Digraph('LogViz', comment=f'Log file visualization for {filename}')
    ...
    def processStack(self, sortedOperations:list):
        ...
        for op in self.operations:   
            ...
            opLabel = opName
            if(idx == 0):
                g.edge('start', op.nodeId, label=f's_{op.idx}')
                continue
            
            elif(idx == len(self.operations) - 1):
                self.g.edge(op.nodeId, 'end', label=f's_{op.idx}')
            else:
                g.edge(op.parent.nodeId, op.nodeId, label=f's_{op.idx}')
